...
int checkPasswd(char *name, char *passwd)
{
    ...
    if(shd!=NULL){
        ...
        if(j<3)
            return -1;
        if(strcmp(crypt(passwd,salt),shd->sp_pwdp)==0)      //passwd：待加密字符串 ； salt：影响加密结果的字符串； 调用crypt进行计算，得到加密密码字段； 在调用strcmp进行比较，‘=0’代表一样
            return 0;
    }
    return -1;
}
...
void handlePORT(Ftpd *ftp)      // PROT h1,h2,h3,h4,p1,p2
    ...
    if((p=strrchr(ftp->arg,','))==NULL)     //strrchr(const char *s, int c): 表示在字符串s中从后到前查找字符c，返回字符c第一次在字符串s中出现的位置，如果未找到字符c，则返回NULL
        goto ERR;
	*p='\0';
