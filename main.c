...
if((port+=(
atoi(p
+1)<<8))>0xffff)       // <<8 左移8比特 因为是p1，p1需要乘以256
goto
ERR;
for(
p = ftp->arg;
*p!='\0';p++)        // h1,h2,h3,h4 变为 h1.h2.h3.h4
if(*p==',')
*
p = '.';
bzero(ftp
->port_addr,sizeof(struct sockaddr_in));
ftp->port_addr->
sin_family = AF_INET;
ftp->port_addr->
sin_port = htons(port);
if(
inet_pton(AF_INET, ftp
->arg,&ftp->port_addr->sin_addr)<=0)
goto
ERR;
writeMsg(ftp,
200,"PORT successful");
